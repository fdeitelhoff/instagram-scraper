...
const start = async function(purgeData) {
    ...
    (err, results) => {
      if (err) {
        ...
        if (resume) {
          ...
          db.all(
            "SELECT Shortcode, UserId, Username, UserFullName, Url, ImageUrl, LikesCount, Created, Complete FROM Posts WHERE Chunk = 'J2' AND Complete = 0 AND Error IS NULL;",
            (err, results) => {
              if (err) {
                console.log(err);
              } else {
  ...
  async function resumeData() {
    db.get(
      `SELECT Shortcode, UserId, Username, UserFullName, Url, ImageUrl, LikesCount, Created, Complete FROM Posts WHERE Shortcode = "${
        resume.Shortcode
  ...
  async function checkPostError(postInfo) {
    try {
      ...
      db.run(
        `UPDATE Posts SET Error = ${error.response.status} WHERE Shortcode = "${
          postInfo.Shortcode
        }"`
      );

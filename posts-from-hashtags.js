...
const start = async function() {
  ...
  async function getHashtagData(
    ...
    try {
      ...
      const pageInfo =
        response.data.data.hashtag.edge_hashtag_to_media.page_info;
      ...
      for (const post of edges) {
        ...
        db.get(
          `SELECT Count(1) as PostExists FROM Posts WHERE Shortcode = "${
            postInfo.shortcode
          }"`,
          async (err, results) => {
            if (err) {
  ...
  async function updatePost(hashtag, shortcode) {
    console.log(
      `Updating post for shortcode ${shortcode} with hashtag ${hashtag}...`
    );

    db.run(
      `UPDATE Posts SET HashtagSearch = "${hashtag}", Chunk = 'J3' WHERE Shortcode = "${shortcode}"`
    );
  }
